...
public class CotizacionV1Service {
    ...
    public CotizacionV1Service(CotizacionRepository cotizacionRepository, CotizacionDetalleRepository detalleRepository, ClienteRepository clienteRepository, UsuarioRepository usuarioRepository, ProductoRepository productoRepository, EstadoCotizacionRepository estadoCotizacionRepository, @Value("${app.business.igv-rate:0.18}") BigDecimal igvRate) {
    ...
    public CotizacionV1Response create(CotizacionCreateRequest request) {
        ...
        c.flagCubierto = request.flagCubierto();
        ...
        c.importeTotal = c.subtotal.add(c.igv);
    ...
    private CotizacionV1Response map(Cotizacion c) {
        List<DetalleCotizacionV1Response> detalles = detalleRepository.findByCotizacionIdCotizacion(c.idCotizacion).stream()
                .map(d -> new DetalleCotizacionV1Response(d.idDetalle, c.idCotizacion, d.producto.idProducto, d.producto.nombreProducto, d.cantidad, d.precioUni == null ? d.precioUnitarioAplicado : d.precioUni)).toList();
        return new CotizacionV1Response(c.idCotizacion, c.cliente.idCliente, c.cliente.razonSocial, c.vendedor == null ? null : c.vendedor.idUsuario,
                c.vendedor == null ? null : c.vendedor.nombres, c.fechaEmision, c.fechaVencimiento, c.moneda, c.subtotal, c.igv,
                c.importeTotal == null ? c.montoTotal : c.importeTotal, c.direccionDespacho, c.depProvDis, c.flagCubierto,
                c.observaciones, c.estadoCotizacionRef == null ? null : c.estadoCotizacionRef.idEstadoCotizacion,
                c.estadoCotizacionRef == null ? c.estadoCotizacion : c.estadoCotizacionRef.descEstadoCotizacion, c.pdfPath, detalles);
    }
}
